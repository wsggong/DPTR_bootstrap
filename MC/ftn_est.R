...
lag_dpd_inst <- function(y, n, t, lb=NULL, ub=NULL, t0=NULL, cut=FALSE, gamma=NULL, q=NULL){
  ...
  if(cut==FALSE){
    ...
    res1 <- res2 <- matrix(0, nrow=n*(t-t0+1), ncol=dimz)
    Y <- matrix(y, nrow=n, ncol=t)
    Q <- matrix(q, nrow=n, ncol=t)
    
    iold <- inew <- 1
    for(i in t0:t){
      res_t <- Y[,max(i-ub,1):(i-lb)]
      inew <- iold+((i-lb)-max(i-ub,1))
      res1[(n*(i-t0)+1):(n*(i-t0+1)),iold:inew] <- res_t
      iold <- inew+1
    }
    
    iold <- inew <- 1
    for(i in t0:t){
      res_t <- Y[,max(i-ub,1):(i-lb)]*(Q[,i]>gamma)
      inew <- iold+((i-lb)-max(i-ub,1))
      res2[(n*(i-t0)+1):(n*(i-t0+1)),iold:inew] <- res_t
      iold <- inew+1
    }
...
gmm_linear_est <- function(y,                     # Dependent Variable
  ...
  return(list(coefficients=coef, objective=obj))
}
