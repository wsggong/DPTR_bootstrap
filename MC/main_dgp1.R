...
for(param_iter1 in 1:length(gamma_grid)){
  for(param_iter2 in 1:length(delta1_grid)){
    ...
    param0 <- c(beta[-1],delta,gamma) # theta0
    
    t1 <- Sys.time()
    
    # registering cluster for parallelization
    cl <- makeCluster(nc) 
    registerDoParallel(cl)
    registerDoRNG(200) #RNG
    
    res <- foreach(i=1:M, .combine=rbind, .errorhandling="pass") %dopar% { # parallelization of loop
      ...
      # Setting parameters for continuity test bootstrap
      ...
      for(b in 1:B1){ # Continuity test bootstrap
      ...
      Chat <- quantile(LRsims, .95) # level 5% cv for continuity test
      Chat2 <- quantile(LRsims, .50) # Used for R-B
      ...
      DT <- 1*(dist > qchisq(.95, df=1)) # testing H0:gamma=gamma0 (1 if reject)
      ...
      # 1. For residual bootstrap
      Ratio <- min(LR/(Chat2*n^(1/4)),1)
      alpha_null2 <- Ratio*alpha_hat+(1-Ratio)*alpha_kink
      gamma_null2 <- Ratio*gamma_hat+(1-Ratio)*gamma_kink
      
      # 2. For grid bootstrap
      alpha_null4 <- alpha_tilde
      gamma_null4 <- gamma
      ...
      for(b in 1:B){
        ...
        boot_res[b,(1):(pp)] <- c(uncon_boot_est$coefficients, uncon_boot_est$threshold) - c(alpha_null2, gamma_null2)
        ...
        booted <- boot_null(inst, y, x, x, q, alpha_null4, alpha_hat, gamma_null4, gamma_hat, n=n, t=t, rng=istar)
        Z_star <- booted$Z
        Y_star <- booted$Y
        x1_star <- booted$x1
        x2_star <- booted$x2
        ...
        con_boot_est <- boot_unconstrained_est(Y_star, x1_star, x2_star, q_star, gamma_null4, g_bar0=g_bar, n=n, t=t, inst=Z_star, weight=Wn_star, two_step=FALSE)
        Ln_tilde_star <- con_boot_est$objective
      ...
      cvs_d <- sd_hat*qnorm(.975) # cv for jump asymptotics
      cvs_c <- c(apply(abs(sim), 1, quantile, .95)/sqrt(n), sqrt(quantile(v, .95)/sqrt(n)))  # cv for kink asymptotics
    ...
    # cv for symmetric percentile CI
    cvs_perc2 <- res[,(pp+1):(2*pp)] # R-B
    cvs_naive <- res[,(2*pp+1):(3*pp)] # NP-B
    
    # cv for percentile CI (upper & lower quantiles)
    cvs_perc2_u <- res[,(3*pp+1):(4*pp)] # R-B(S)
    cvs_naive_u <- res[,(4*pp+1):(5*pp)] # NP-B(S)
    
    cvs_perc2_l <- res[,(5*pp+1):(6*pp)]
    cvs_naive_l <- res[,(6*pp+1):(7*pp)]
    
    # cv for grid bootstrap
    cvs_grid <- res[,7*pp+1]
    ...
    # asymptotic CI
    acvs_c <- res[,(7*pp+2):(8*pp+1)] # for continuous model
    acvs_d <- res[,(8*pp+2):(9*pp+1)] # for discontinuous model
    
    DT <- res[,9*pp+2] # threshold location distance test result
    
    Chat <- res[,9*pp+3] # continuity test result (bootstrap)
    dist <- res[,9*pp+4] # threshold location test statistics
    LR <- res[,9*pp+5] # continuity test statistics
    ...
    filename <- paste("boot_n",n,"_g",gamma,"_p",delta1_grid[param_iter2],".Rdata",sep="")
    save.image(filename)
